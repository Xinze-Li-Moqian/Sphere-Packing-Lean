...
> That the supremum over packings of unit density is at most the sphere packing constant is obvious. For the reverse inequality, let $\mathcal{P}(X)$ be any sphere packing with separation radius $r$. We know, from Lemma~[[spherepackingscale-density]], that the density of $\mathcal{P}(X)$ is equal to that of the scaled packing $\mathcal{P}\!\left(\frac{X}{r}\right)$. Since the scaled packing has separation radius $1$, its density is naturally at most the supremum over all packings of unit density, meaning that the same is true of $\mathcal{P}(X)$.
